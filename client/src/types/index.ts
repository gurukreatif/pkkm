...
export type UserRole = 'admin' | 'pengawas' | 'operator';

export interface DataMadrasah {
  namaMadrasah: string; status: 'Negeri' | 'Swasta'; npsn: string;
  alamat: string; desa: string; kecamatan: string; kabupatenKota: string;
  provinsi: string; telp?: string; email?: string;
}
export interface DataKepala {
  nama: string; nip: string; jenisKelamin: 'Laki - laki' | 'Perempuan';
  tempatTanggalLahir: string; jabatan: string; pangkatGolongan: string;
  tmtKepalaMadrasah?: string; masaKerjaTahun?: number; masaKerjaBulan?: number;
  nuptk?: string; pendidikanTerakhir: string;
}
export interface DataPenilai {
  nama: string; nip: string; pangkatGolongan?: string;
  jabatan?: string; unitKerja?: string; tanggalPenilaian?: string;
}
export interface SkorKompetensi { kode: string; unsur: string; tugas: string; skor: number; }
export interface HasilKompetensi {
  pkks1: number; pkks2: number; pkks3: number; pkks4: number;
  nilaiAkhir: number; predikat: Predikat;
}
export interface PenilaianTahunan {
  tahunKe: 1 | 2 | 3 | 4; tahunPenilaian?: string;
  pengawas1?: DataPenilai; pengawas2?: DataPenilai;
  skorDetail: SkorKompetensi[]; hasilKompetensi: HasilKompetensi;
  nilaiGuru: number; nilaiTendik: number; nilaiKomite?: number;
  nilaiAkhirTahun: number; predikat: Predikat; catatanPengawas?: string;
  tanggalSelesai?: string; status: 'draft' | 'selesai';
}
export interface NilaiKumulatif {
  tahun1: number; tahun2: number; tahun3: number; tahun4: number;
  nkkm: number; predikatAkhir: Predikat;
}
export interface PenilaianKepala {
  id?: string; madrasah: DataMadrasah; kepala: DataKepala; tim: DataPenilai[];
  penilaianTahunan: PenilaianTahunan[]; nilaiKumulatif?: NilaiKumulatif;
  tahunAktif: 1 | 2 | 3 | 4; createdAt?: string; updatedAt?: string;
  createdBy?: string;
  // RBAC: npsn sekolah yang boleh akses (untuk operator)
  npsn?: string;
}
export interface User {
  uid: string; email: string; displayName?: string;
  role: UserRole; kanwil?: string;
  // Untuk operator/sekolah — NPSN madrasah mereka
  npsn?: string;
}

export const PREDIKAT_THRESHOLD = { 'Amat Baik': 91, 'Baik': 76, 'Cukup': 61, 'Sedang': 51, 'Kurang': 0 } as const;
...
export const KOMPETENSI = [
  { kode: '1', nama: 'Usaha Pengembangan Madrasah', alias: 'PKKS 1', subKompetensi: [
  ...
  { kode: '2', nama: 'Pelaksanaan Tugas Manajerial', alias: 'PKKS 2', subKompetensi: [
  ...
  { kode: '3', nama: 'Pengembangan Kewirausahaan', alias: 'PKKS 3', subKompetensi: [
  ...
  { kode: '4', nama: 'Supervisi Kepada Guru dan Tenaga Kependidikan', alias: 'PKKS 4', subKompetensi: [
